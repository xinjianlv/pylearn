...
int2OneHotArray = np.zeros(dim)


def int2OneHot(x):
    int2OneHotArray[x] = 1
    return int2OneHotArray
def oneHot2int(x):
...
for i in range(len(data)):
    index = []
    for t in data[i]:
        if t in wordDic.keys():
            index.append(wordDic[t])
    Xline = copy.deepcopy(index)
    yline = copy.deepcopy(index)
    Xline.insert(0 , 0)
    yline.append(1)
    out = []
    overallError = 0
    ...
    for position in range(slength):
        X = np.array([int2OneHot(Xline[position])])
        y = np.array([int2OneHot(yline[position])])
        ...
        for ndx in range( len(layer_2[0])):
            bitvalue.append( int(np.round(layer_2[0][ndx])))
        c = oneHot2int(bitvalue)
        out.append(c)
